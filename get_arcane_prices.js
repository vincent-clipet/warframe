...
for (const arcane of arcanes) {
    ...
    for (const stat of statistics) {
        if (stat.mod_rank === 5) {
            pricing_data.volume_rank5 += stat.volume ? stat.volume : 0
            pricing_data.moving_avg_rank5 = (pricing_data.moving_avg_rank5 === 0 && stat.moving_avg) ? stat.moving_avg : pricing_data.moving_avg_rank5
        } else if (stat.mod_rank === 0) {
            pricing_data.volume_rank0 += stat.volume ? stat.volume : 0
            pricing_data.moving_avg_rank0 = (pricing_data.moving_avg_rank0 === 0 && stat.moving_avg) ? stat.moving_avg : pricing_data.moving_avg_rank0
        }
    }

    const str = [
        arcane.name,
        pricing_data.moving_avg_rank0.toString().replace('.', ','),
        pricing_data.volume_rank0,
        pricing_data.moving_avg_rank5.toString().replace('.', ','),
        pricing_data.volume_rank5,
    ].join("\t")
    lines.push(str)
    console.log(str)
